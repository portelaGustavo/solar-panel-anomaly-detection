...
\subsection{Análise do melhor modelo (XGBoost)}
...
Dois padrões se destacam. Primeiro, vários tipos de anomalia sutil --- \textit{Hot-Spot} (56\%), \textit{Offline-Module} (44\%) e \textit{Shadowing} (34\%) --- são frequentemente confundidos com \textit{No-Anomaly}, indicando que sua assinatura térmica é fraca e próxima da de um módulo nominal. Segundo, há confusão entre classes "irmãs", de mesmo defeito em grau diferente, como \textit{Diode-Multi} com \textit{Diode} (37\%) e \textit{Cell-Multi} com \textit{Cell} (20\%). A classe \textit{No-Anomaly}, por sua vez, é classificada com excelência (96\% de acerto), reforçando a robustez do modelo como detector de falhas.
